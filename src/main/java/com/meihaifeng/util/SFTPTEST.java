...
public class SFTPTEST {
    ...
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(SFTPTEST.class);
...
//        config.put("UseDNS", "no");
    ...
    public static void main(String args[]) throws Exception {
        Session session = longConnect("115.159.160.97","drore","Y*6xUE9dt?4%K",22);
        ChannelSftp channelSftp = openChannelSftp(session);
        String remotePath = "/mnt/drore/gismanage/gis/webapps/ROOT/images/";
        Vector vector = listFiles(session,remotePath);
        for (int i=0;i<vector.size();i++){
            ...
            URL http_url = new URL("http://115.159.160.97:7201/images/"+fileName);
            InputStream is = http_url.openStream();
            BufferedImage bufferedImage = ImageIO.read(is);
            System.out.println(fileName+"     width     "+bufferedImage.getWidth());
            System.out.println(fileName+"     height    "+bufferedImage.getHeight());
        }
        close(session);
    }
     ...
     */
    public static void listFilesInputStream(Session session, String remotePath) throws Exception {
        ChannelSftp channelSftp = openChannelSftp(session);
        try {
            ...
            for (int i=0;i<vector.size();i++){
                ...
                InputStream inputStream = channelSftp.get(remotePath+fileName);
